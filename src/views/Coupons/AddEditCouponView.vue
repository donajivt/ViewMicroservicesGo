<template>
    ...
    <div class="container mt-2">
        <h2 class="mb-4">{{ isEditing ? 'Editar Cupón' : 'Agregar Nuevo Cupón' }}</h2>
        <form @submit.prevent="submitForm" class="add-edit-form">
            <div class="row">
                <!-- Columna izquierda -->
                <div class="col-md-6">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input v-model.number="coupon.discountAmount" type="number" class="form-control"
                            id="discountAmount" required min="0" />
                    </div>

                    <div class="mb-3">
                        <label for="minAmount" class="form-label">Monto Mínimo</label>
                        <input v-model.number="coupon.minAmount" type="number" class="form-control" id="minAmount"
                            min="0" />
                    </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input v-model.number="coupon.limitUse" type="number" class="form-control" id="limitUse"
                            min="1" />
                    </div>
            ...
            <div class="d-flex justify-content-between mt-4">
                ...
                <button type="submit" class="btn btn-add">
                    ...
                    {{ isEditing ? 'Actualizar Cupón' : 'Agregar Cupón' }}
                </button>
            </div>
        </form>
    </div>
...
onMounted(async () => {
    if (isEditing.value) {
        try {
            const response = await fetch(`http://localhost:7777/api/coupons/${route.params.id}`, {
                headers: {
            ...
            if (data.is_success) {
                ...
                coupon.value.dateInit = formatDateForInput(coupon.value.dateInit);
                coupon.value.dateEnd = formatDateForInput(coupon.value.dateEnd);
            }
        } catch (error) {
...
const submitForm = async () => {
    try {
        const payload = {
            ...coupon.value,
            dateInit: toISOStringOrNull(coupon.value.dateInit),
            dateEnd: toISOStringOrNull(coupon.value.dateEnd)
        };

        const url = isEditing.value
            ? `http://localhost:7777/api/coupons/${route.params.id}`
            : 'http://localhost:7777/api/coupons/';
